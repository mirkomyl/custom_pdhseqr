...
*                                                                     *
***********************************************************************
      RECURSIVE SUBROUTINE PDLAQR0_( WANTT, WANTZ, N, ILO, IHI, H,
...
*  =======
*
*  PDLAQR0_ computes the eigenvalues of a Hessenberg matrix H
...
*          set by a previous call to PDGEBAL_, and then passed to PDGEHRD
*          when the matrix output by PDGEBAL_ is reduced to Hessenberg
...
*                   value
*          .GT. 0:  if INFO = i, PDLAQR0_ failed to compute all of
...
*     than or equal to six (6) due to restrictions in PDLAQR1_, PDLAQR5_
*     and DLAQR6_. Moreover, H and Z need to be distributed identically
...
*     ..
*     .. External Functions ..
      INTEGER            PILAENVX_, NUMROC, INDXG2P, ICEIL, BLACS_PNUM
      EXTERNAL           PILAENVX_, NUMROC, INDXG2P, ICEIL, BLACS_PNUM
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
      EXTERNAL           PDLACPY, PDLAQR1_, DLANV2, PDLAQR3_, PDLAQR5_,
...
*
      ...
      END IF
*
*     Set up job flags for PILAENVX_.
*
      IF( WANTT ) THEN
         JBCMPZ( 1: 1 ) = 'S'
      ELSE
...
*
      ...
      NB = DESCH( MB_ )
*
*     Tiny (sub-) matrices must use PDLAQR1_. (Stops recursion)
*
      IF( N.LE.NTINY ) THEN
*
*     Estimate optimal workspace.
*
         CALL PDLAQR1_( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR, WI,
...
*        NWR.GE.3.)
*
         NWR = PILAENVX_( ICTXT, 13, 'PDLAQR0_', JBCMPZ, N, ILO, IHI,
     $        LWORK )
         NWR = MAX( 2, NWR )
...
*
         NWIN = PILAENVX_( ICTXT, 19, 'PDLAQR0_', JBCMPZ, N, NB, NB, NB)
         NSR = PILAENVX_( ICTXT, 15, 'PDLAQR0_', JBCMPZ, N, ILO, IHI,
     $        MAX(NWIN,NB) )
         NSR = MIN( NSR, IHI-ILO )
...
*        Estimate optimal workspace
*
*        Workspace query call to PDLAQR0_/PDLAQR1_
*
         CALL PDLAQR1_( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR, WI,
     $        ILOZ, IHIZ, Z, DESCZ, WORK, -1, IWORK, -1, INFO )
         ...
         IF( RECLEVEL.LT.RECMAX ) THEN
            CALL PDLAQR0_( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR, WI,
     $           ILOZ, IHIZ, Z, DESCZ, WORK, -1, IWORK, -1, INFO,
     $           RECLEVEL+1 )
...
*
*        Workspace query call to PDLAQR3_
*
         CALL PDLAQR3_( WANTT, WANTZ, N, ILO, IHI, NWR+1, H,
     ...
     $        -1, RECLEVEL )
         ...
         LIWKOPT = MAX( LIWKOPT, IWORK( 1 ) )
*
*        Workspace query call to PDLAQR5_
*
         CALL PDLAQR5_( WANTT, WANTZ, 2, N, 1, N, N, WR, WI, H,
     $        DESCH, ILOZ, IHIZ, Z, DESCZ, WORK, -1, IWORK, -1 )
*
*        Optimal workspace = MAX(PDLAQR3_, PDLAQR5_)
*
...
*
         ...
         END IF
*
*        PDLAQR1_/PDLAQR0_ crossover point.
*
         NMIN = PILAENVX_( ICTXT, 12, 'PDLAQR0_', JBCMPZ, N, ILO, IHI,
     $        LWORK )
         NMIN = MAX( NTINY, NMIN )
*
*        Nibble crossover point.
*
         NIBBLE = PILAENVX_( ICTXT, 14, 'PDLAQR0_', JBCMPZ, N, ILO, IHI,
     $        LWORK )
         NIBBLE = MAX( 0, NIBBLE )
*
...
*
         KACC22 = PILAENVX_( ICTXT, 16, 'PDLAQR0_', JBCMPZ, N, ILO, IHI,
     $        LWORK )
         KACC22 = MAX( 1, KACC22 )
         KACC22 = MIN( 2, KACC22 )
*
...
*
            IWORK(1) = IT
            CALL PDLAQR3_( WANTT, WANTZ, N, KTOP, KBOT, NW, H,
...
*              This may be lowered (slightly) if PDLAQR3_
...
*              PDLAQR3_ above or from the eigenvalues
...
*
 ...
 30               CONTINUE
     ...
     $                    KS+1, DESCH )
               ...
               ELSE
*
*                 Got NS/2 or fewer shifts? Use PDLAQR0_ or
*                 PDLAQR1_ on a trailing principal submatrix to
*                 get more.
*
                  IF( KBOT-KS+1.LE.NS / 2 ) THEN
                     KS = KBOT - NS + 1
                     KT = N - NS + 1
                     NPMIN = PILAENVX_( ICTXT, 23, 'PDLAQR0_', 'EN', NS,
     $                    NB, NPROW, NPCOL )
c
c   Temporarily force NPMIN <= 8 since only PDLAQR1_ is used.
c
                     NPMIN = MIN(NPMIN, 8)
...
*
     ...
     $                          DESCT )
                           CALL PDLAQR0_( .FALSE., .FALSE., IROFFH+NS,
     ...
     $                          IERR, RECLEVEL+1 )
                        ELSE
                           CALL PDLAMVE_( 'All', NS, NS, H, KS, KS,
     $                          DESCH, WORK(IPT), 1+IROFFH, 1+ICOFFH,
     $                          DESCT, WORK(IPWRK) )
                           CALL PDLAQR1_( .FALSE., .FALSE., IROFFH+NS,
     $                          1+IROFFH, IROFFH+NS, WORK(IPT),
     $                          DESCT, WR( KS-IROFFH ),
...
*
               CALL PDLAQR5_( WANTT, WANTZ, KACC22, N, KTOP, KBOT,
