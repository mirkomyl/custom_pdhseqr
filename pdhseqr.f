...
*                                                                     *
*     pdhseqr.f:                                                      *
*         Computational routine in the package PDHSEQR_.               *
*                                                                     *
...
*                                                                     *
***********************************************************************
      SUBROUTINE PDHSEQR_( JOB, COMPZ, N, ILO, IHI, H, DESCH, WR, WI, Z,
...
*  =======
*
*  PDHSEQR_ computes the eigenvalues of an upper Hessenberg matrix H
...
*          .GT. 0:  if INFO = i, PDHSEQR_ failed to compute all of
...
*                accessed.
*
*          = -7777: PDLAQR0_ failed to converge and PDLAQR1_ was called
*                   instead. This could happen. Mostly due to a bug.
*                   Please, send a bug report to the authors.
*          = -8888: PDLAQR1_ failed to converge and PDLAQR0_ was called
...
*     ..
*     .. External Functions ..
      INTEGER            PILAENVX_, NUMROC, ICEIL
      LOGICAL            LSAME
      EXTERNAL           PILAENVX_, LSAME, NUMROC, ICEIL
*     ..
*     .. External Subroutines ..
      EXTERNAL           PDLACPY, PDLAQR1_, PDLAQR0_, PDLASET, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL PXERBLA( ICTXT, 'PDHSEQR_', -INFO )
         RETURN
*
      ELSE IF( N.EQ.0 ) THEN
*
...
*
      ELSE
*
*        Copy eigenvalues isolated by PDGEBAL_.
*
         DO 10 I = 1, ILO - 1
...
*
         NH = IHI-ILO+1
         NMIN = PILAENVX_( ICTXT, 12, 'PDHSEQR_',
     $        JOB( : 1 ) // COMPZ( : 1 ), N, ILO, IHI, LWORK )
         NMIN = MAX( NTINY, NMIN )
*
*        PDLAQR0_ for big matrices; PDLAQR1_ for small ones.
*
...
*
*              A rare PDLAQR0_ failure!  PDLAQR1_ sometimes succeeds
*              when PDLAQR0_ fails.
*
               KBOT = INFO
               CALL PDLAQR1_( WANTT, WANTZ, N, ILO, IHI, H, DESCH, WR,
...
*
            IF( INFO.GT.0 ) THEN
*
*              A rare PDLAQR1_ failure!  PDLAQR0_ sometimes succeeds
*              when PDLAQR1_ fails.
*
               KBOT = INFO
*
...
*                 space to call PDLAQR0_ directly.
*
                  CALL PDLAQR0_( WANTT, WANTZ, N, ILO, KBOT, H, DESCH,
     $                 WR, WI, ILO, IHI, Z, DESCZ, WORK, LWORK,
     $                 IWORK, LIWORK, INFO, 0 )
               ELSE
*
*                 Tiny matrices don't have enough subdiagonal
*                 scratch space to benefit from PDLAQR0_.  Hence,
*                 tiny matrices must be copied into a larger
*                 array before calling PDLAQR0_.
*
                  HROWS = NUMROC( NL, NB, MYROW, DESCH(RSRC_), NPROW )
                  HCOLS = NUMROC( NL, NB, MYCOL, DESCH(CSRC_), NPCOL )
     ...
     $                 N+1, DESCH2 )
                  ...
                  CALL PDLAQR0_( WANTT, WANTZ, NL, ILO, KBOT, WORK,
...
*
     ...
     $              DUM4, CS, SN )
               IF( TMP3.EQ.ZERO ) THEN
                  IF( WANTT ) THEN
                     IF( I+2.LE.N )
     $                  CALL PDROT_( N-I-1, H, I, I+2, DESCH,
     $                       DESCH(M_), H, I+1, I+2, DESCH, DESCH(M_),
     $                       CS, SN, WORK, LWORK, IERR )
                     CALL PDROT_( I-1, H, 1, I, DESCH, 1, H, 1, I+1,
     $                    DESCH, 1, CS, SN, WORK, LWORK, IERR )
                  END IF
                  IF( WANTZ ) THEN
                     CALL PDROT_( IHI-ILO+1, Z, ILO, I, DESCZ, 1,
