...
*                                                                     *
***********************************************************************
      SUBROUTINE BDTREXC_( N, T, LDT, IFST, ILST, NITRAF, ITRAF,
...
*  =======
*
*  BDTREXC_ reorders the real Schur factorization of a real matrix
...
*     ..
*     .. Local Scalars ..
      INTEGER            CDTRAF, CITRAF, LDTRAF, LITRAF, HERE, NBF,
     $                   NBL, NBNEXT
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
            ...
            IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
               ...
               RETURN
            END IF
            CALL BDLAEXC_( N, T, LDT, HERE, NBF, NBNEXT, ITRAF(CITRAF),
...
*
     ...
     $            NBNEXT = 2
            ...
            IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
               ...
               RETURN
            END IF
            CALL BDLAEXC_( N, T, LDT, HERE+1, 1, NBNEXT, ITRAF(CITRAF),
     $                    DTRAF(CDTRAF), WORK, INFO )
            IF( INFO.NE.0 ) THEN
               ILST = HERE
               NITRAF = CITRAF - 1
...
*
               ...
               IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
                  ...
                  RETURN
               END IF
               CALL BDLAEXC_( N, T, LDT, HERE, 1, NBNEXT, ITRAF(CITRAF),
     $                       DTRAF(CDTRAF), WORK, INFO )
               CITRAF = CITRAF + LITRAF
               CDTRAF = CDTRAF + LDTRAF
               HERE = HERE + 1
...
*
                  ...
                  IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
                     ...
                     RETURN
                  END IF
                  CALL BDLAEXC_( N, T, LDT, HERE, 1, NBNEXT,
...
*
                  ...
                  IF( CDTRAF+2*LDTRAF-1.GT.NDTRAF ) THEN
                     ...
                     RETURN
                  END IF
                  CALL BDLAEXC_( N, T, LDT, HERE, 1, 1, ITRAF(CITRAF),
     $                          DTRAF(CDTRAF), WORK, INFO )
                  CITRAF = CITRAF + LITRAF
                  CDTRAF = CDTRAF + LDTRAF
                  CALL BDLAEXC_( N, T, LDT, HERE+1, 1, 1, ITRAF(CITRAF),
     $                          DTRAF(CDTRAF), WORK, INFO )
                  CITRAF = CITRAF + LITRAF
                  CDTRAF = CDTRAF + LDTRAF
                  HERE = HERE + 2
...
*
            ...
            IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
               ...
               RETURN
            END IF
            CALL BDLAEXC_( N, T, LDT, HERE-NBNEXT, NBNEXT, NBF,
...
*
     ...
     $            NBNEXT = 2
            ...
            IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
               ...
               RETURN
            END IF
            CALL BDLAEXC_( N, T, LDT, HERE-NBNEXT, NBNEXT, 1,
     $                    ITRAF(CITRAF), DTRAF(CDTRAF), WORK, INFO )
            IF( INFO.NE.0 ) THEN
               ILST = HERE
               NITRAF = CITRAF - 1
...
*
               ...
               IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
                  ...
                  RETURN
               END IF
               CALL BDLAEXC_( N, T, LDT, HERE, NBNEXT, 1, ITRAF(CITRAF),
     $                       DTRAF(CDTRAF), WORK, INFO )
               CITRAF = CITRAF + LITRAF
               CDTRAF = CDTRAF + LDTRAF
               HERE = HERE - 1
...
*
                  ...
                  IF( CDTRAF+LDTRAF-1.GT.NDTRAF ) THEN
                     ...
                     RETURN
                  END IF
                  CALL BDLAEXC_( N, T, LDT, HERE-1, 2, 1, ITRAF(CITRAF),
     $                          DTRAF(CDTRAF), WORK, INFO )
                  IF( INFO.NE.0 ) THEN
                     INFO = 2
                     ILST = HERE
