...
*                                                                     *
***********************************************************************
      SUBROUTINE PDTRORD_( COMPQ, SELECT, PARA, N, T, IT, JT,
...
*  =======
*
*  PDTRORD_ reorders the real Schur factorization of a real matrix
...
*
*  T must be in Schur form (as returned by PDHSEQR_), that is, block
...
*          to PILAENVX_ and others by meaningful default values
*          in future releases):
...
*  since PDLAHQR_ does not compute Schur forms of submatrices anyway.
*
*  References
*  ==========
*
...
*
      IF( INFO.NE.0 .AND. .NOT.LQUERY ) THEN
         M = 0
         CALL PXERBLA( ICTXT, 'PDTRORD_', -INFO )
         RETURN
      ELSEIF( LQUERY ) THEN
         WORK( 1 ) = DBLE(LWMIN)
         IWORK( 1 ) = LIWMIN
...
*
     ...
     $                 WORK( PDW ), NWIN )
                  CALL BDLAAPP_( 1, NWIN, NWIN, NCB, WORK( PDW ), NWIN,
...
*
     ...
     $                       MYROW, MYCOL, ILOC, JLOC, RSRC1, CSRC1 )
                        IF( MYROW.EQ.RSRC1 .AND. MYCOL.EQ.CSRC1 ) THEN
                           LROWS = MIN(NB,I-INDX)
                           CALL BDLAAPP_( 1, LROWS, NWIN, NCB,
 ...
 280                    CONTINUE
     ...
     $                          THEN
                              LCOLS = MOD( MIN( NB-MOD(LIHI,NB),
     $                             N-LIHI ), NB )
                              CALL BDLAAPP_( 0, NWIN, LCOLS, NCB,
     $                             T((JLOC-1)*LLDT+ILOC), LLDT, NITRAF,
     $                             IWORK(IPIW), WORK( IPW2 ),
     $                             WORK(IPW3) )
                           END IF
...
*
         IF( IERR.NE.0 ) THEN
*
*           When calling BDTREXC_, the block at position I+KKS-1 failed
*           to swap.
*
...
*
                              ...
                              IF( KK.NE.KS ) THEN
                                 NITRAF = LIWORK - PITRAF + 1
                                 NDTRAF = LWORK - PDTRAF + 1
                                 CALL BDTREXC_( NWIN, WORK(IPW2), NWIN,
...
*
     ...
     $                       WORK( IPW4 ), NWIN )
                        ...
                        CALL BDLAAPP_( 1, NWIN, NWIN, NCB, WORK( IPW4 ),
...
*
     ...
     $                             THEN
                                 CALL BDLAAPP_( 1, TROWS, NWIN, NCB,
 ...
 500                       CONTINUE
     ...
     $                                RSRC, CSRC )
                                 IF( MYROW.EQ.RSRC .AND. MYCOL.EQ.CSRC )
     $                                THEN
                                    CALL BDLAAPP_( 1, QROWS, NWIN, NCB,
...
*
     ...
     $                            MOD(LIHIC,NB).NE.0 ) THEN
                                 CALL BDLAAPP_( 0, NWIN, TCOLS, NCB,
