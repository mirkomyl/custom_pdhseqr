...
*                                                                     *
***********************************************************************
      RECURSIVE SUBROUTINE PDLAQR1_( WANTT, WANTZ, N, ILO, IHI, A,
...
*  =======
*
*  PDLAQR1_ is an auxiliary routine used to find the Schur decomposition
*    and or eigenvalues of a matrix already in Hessenberg form from
*    cols ILO to IHI.
*
*  This is a modified version of PDLAHQR_ from ScaLAPACK version 1.7.3.
...
*    o For some reason, the original version of PDLAHQR_ sometimes did
...
*          ILO = 1). PDLAQR1_ works primarily with the Hessenberg
...
*          matrix Z of transformations accumulated by PDHSEQR_, and on
...
*          = 0: successful exit
*          > 0: PDLAQR1_ failed to compute all the eigenvalues ILO to IHI
...
*                        back.
*           DLAREF_    -> Row/column reflector applier. Core routine here.
...
*     .. Local Scalars ..
     ...
     $                   DBLK, NIBBLE, ND, NS, LTOP, LWKOPT, S1, S2, S3
      DOUBLE PRECISION   AVE, DISC, H10, H11, H12, H21, H22, H33,
...
*     .. External Subroutines ..
     ...
     $                   DGERV2D, DGESD2D, DGSUM2D, DLAHQR, DLAREF_,
     $                   DLARFG, DLASORTE, IGAMX2D, INFOG1L, INFOG2L,
     $                   PDLABAD, PDLACP3, PDLASMSUB,
     $                   PDLAWIL, PXERBLA, DLANV2, PDLAQR2_, PDLAQR4_
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $             +6*LDS*LDS )
      ...
      IF( HBL.LT.6 ) THEN
         INFO = -( 700+MB_ )
      END IF
      CALL IGAMX2D( CONTXT, 'ALL', ' ', 1, 1, INFO, 1, ITMP1, ITMP2, -1,
     $              -1, -1 )
      IF( INFO.LT.0 ) THEN
         CALL PXERBLA( CONTXT, 'PDLAQR1_', -INFO )
         WORK( 1 ) = DBLE( LWKOPT )
         RETURN
      END IF
*
...
*
      IF( NH .LE. LDS ) THEN
         CALL PDLAQR4_( WANTT, WANTZ, N, ILO, IHI, A, DESCA, WR, WI,
...
*
            IF( AED ) THEN
               ...
               CALL PDLAQR2_( WANTT, WANTZ, N, L, I, DBLK, A, DESCA,
...
*
               ...
               I = I - ND
               DBLK = DBLK - ND
               IF( 100*ND .GT. NIBBLE*NH .OR. DBLK .LT. 2*JBLK )
     $            GOTO 10
*
...
*               IF( ND.GE.0 .AND. ND+DBLK.GE.64 ) THEN
               IF( .FALSE. ) THEN
                  IF( NUM.GT.1 .AND. DBLK.GT.0 ) THEN
                     CALL DGSUM2D( CONTXT, 'All', ' ', DBLK, 1,
     $                    WR( I-DBLK+1 ), DBLK, -1, -1 )
                     CALL DGSUM2D( CONTXT, 'All', ' ', DBLK, 1,
     $                    WI( I-DBLK+1 ), DBLK, -1, -1 )
                  END IF
...
*
   ...
   22             CONTINUE
               ...
               ELSE
                  CALL INFOG2L( I-DBLK+1, I-DBLK+1, DESCA, NPROW, NPCOL,
     $                 MYROW, MYCOL, IROW, ICOL, II, JJ )
                  IF ( MYROW.EQ.II .AND. MYCOL.EQ.JJ ) THEN
                     CALL DLAHQR( .FALSE., .FALSE., DBLK, 1, DBLK,
     $                            WORK( S1+1 ), LDS, WORK( IRBUF+1 ),
     $                            WORK( ICBUF+1 ), 1, DBLK, Z, LDZ,
     $                            IERR )
                     IF( NUM.GT.1 ) THEN
                        CALL DGEBS2D( CONTXT, 'All', ' ', DBLK, DBLK,
     $                       WORK( S1+1 ), LDS )
                     END IF
                  ELSE
                     CALL DGEBR2D( CONTXT, 'All', ' ', DBLK, DBLK,
     $                    WORK( S1+1 ), LDS, II, JJ )
                  END IF
               END IF
            ELSE
...
*
                        T2 = T1*V2
                        T3 = T1*V3
                        CALL DLAREF_( 'Col', A, LDA, .FALSE., Z, LDZ,
     ...
     $                      HBL ).LT.HBL-2 ) ) THEN
                           T2 = T1*V2
                           T3 = T1*V3
                           CALL DLAREF_( 'Row', A, LDA, .FALSE., Z, LDZ,
...
*
     ...
     $                          ITMP1, ITMP2 )
                  ...
                  II = KROW( KI )
                  CALL DLAREF_( 'Row', A, LDA, WANTZ, Z, LDZ, .TRUE.,II,
...
*
     ...
     $                      ( NPROW.EQ.1 ) ) THEN
                           T2 = T1*V2
                           T3 = T1*V3
                           CALL DLAREF_( 'Row', A, LDA, WANTZ, Z, LDZ,
     $                                  .FALSE., IROW1, IROW1, ISTART,
...
*
     ...
     $                 IROW1, IROW2 )
                  ...
                  ELSE
                     ITMP2 = -1
                  END IF
                  CALL DLAREF_( 'Col', A, LDA, WANTZ, Z, LDZ, .TRUE.,
...
*
         IF( WANTT ) THEN
            IF(I .LT. N) CALL PDROT_( N-I, A, L, I+1, DESCA,DESCA( M_ ),
     ...
     $                               LWORK-VECSIDX, IERR )
         ...
         ELSE
            LTOP = I1
         END IF
         IF (L .GT. LTOP) CALL PDROT_( L-LTOP, A, LTOP, L, DESCA, 1, A,
     $                                LTOP, I, DESCA, 1, CS, SN,
     $                                WORK( VECSIDX+1 ), LWORK-VECSIDX,
     $                                IERR )
         IF( WANTZ ) THEN
            CALL PDROT_( IHIZ-ILOZ+1, Z, ILOZ, L, DESCZ, 1, Z, ILOZ, I,
...
*
         NH = I - L + 1
         IF( NH .LE. LDS ) THEN
            CALL PDLAQR4_( WANTT, WANTZ, N, L, I, A, DESCA, WR, WI,
     $                    ILOZ, IHIZ, Z, DESCZ, WORK( S1+1 ), NH,
     $                    WORK( S2+1 ), NH, WORK( S3+1 ), 4*LDS*LDS,
     $                    INFO )
            IF( INFO.NE.0 ) THEN
