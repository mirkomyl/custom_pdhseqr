...
*                                                                     *
***********************************************************************
      RECURSIVE SUBROUTINE PDLAQR3_( WANTT, WANTZ, N, KTOP, KBOT, NW, H,
...
*          values of LWORK.
*
*          If LWORK = -1, then a workspace query is assumed; PDLAQR3_
...
*     .. External Functions ..
      ...
      EXTERNAL           DLAMCH, PILAENVX_, NUMROC, INDXG2P, ICEIL,
     $                   BLACS_PNUM
*     ..
*     .. External Subroutines ..
      EXTERNAL           PDCOPY, PDGEHRD, PDGEMM, DLABAD, PDLACPY,
     $                   PDLAQR1_, DLANV2, PDLAQR0_, PDLARF, PDLARFG,
     $                   PDLASET, PDTRORD_, PDELGET, PDELSET,
     $                   PDLAMVE_, BLACS_GRIDINFO, BLACS_GRIDMAP,
...
*
      PAR(1) = PILAENVX_(ICTXT, 17, 'PDLAQR3_', 'SV', JW, NB, -1, -1)
      PAR(2) = PILAENVX_(ICTXT, 18, 'PDLAQR3_', 'SV', JW, NB, -1, -1)
      PAR(3) = PILAENVX_(ICTXT, 19, 'PDLAQR3_', 'SV', JW, NB, -1, -1)
      PAR(4) = PILAENVX_(ICTXT, 20, 'PDLAQR3_', 'SV', JW, NB, -1, -1)
      PAR(5) = PILAENVX_(ICTXT, 21, 'PDLAQR3_', 'SV', JW, NB, -1, -1)
      PAR(6) = PILAENVX_(ICTXT, 22, 'PDLAQR3_', 'SV', JW, NB, -1, -1)
*
...
*
     ...
     $        WORK, V, 1, 1, DESCV, WORK, -1, INFO )
         LWK2 = INT( WORK( 1 ) )
*
*        Workspace query call to PDLAQR0_.
*
         IF( RECLEVEL.LT.RECMAX ) THEN
            CALL PDLAQR0_( .TRUE., .TRUE., JW+IROFFH, 1+IROFFH,
     ...
     $           WORK, -1, IWORK, -1, INFQR, RECLEVEL+1 )
         ...
         DESCT( RSRC_ ) = 0
         DESCT( CSRC_ ) = 0
         CALL PDLAQR1_( .TRUE., .TRUE., JW+IROFFH, 1, JW+IROFFH, T,
...
*
         CALL PDTRORD_( 'Vectors', IWORK, PAR, JW+IROFFH, T, 1, 1,
...
*
      WORK( 1 ) = DBLE( LWKOPT )
*
*     IWORK(1:NSEL) is used as the array SELECT for PDTRORD_.
*
...
*
     ...
     $         CALL PDROT_( N-I-1, H, I, I+2, DESCH, DESCH(M_), H, I+1,
     $              I+2, DESCH, DESCH(M_), CS, SN, WORK, LWORK, INFO )
            IF( I.GT.1 )
     $         CALL PDROT_( I-1, H, 1, I, DESCH, 1, H, 1, I+1, DESCH, 1,
     $              CS, SN, WORK, LWORK, INFO )
            IF( WANTZ )
     $         CALL PDROT_( IHIZ-ILOZ+1, Z, ILOZ, I, DESCZ, 1, Z, ILOZ,
...
*
      NPMIN = PILAENVX_( ICTXT, 23, 'PDLAQR3_', 'SV', JW, NB, NPROW,
     $     NPCOL )
      NMIN = PILAENVX_( ICTXT, 12, 'PDLAQR3_', 'SV', JW, 1, JW, LWORK )
      NMAX = ( N-1 ) / 3
...
*
         ...
         ELSE
            IF( DESCT(RSRC_).EQ.0 .AND. DESCT(CSRC_).EQ.0 ) THEN
               IF( JW+IROFFH.GT.DESCT( MB_ ) ) THEN
                  CALL PDLAQR1_( .TRUE., .TRUE., JW+IROFFH, 1,
     $                 JW+IROFFH, T, DESCT, SR( KWTOP-IROFFH ),
     ...
     $              0, ICTXT, MAX(1,TZROWS0), IERR0 )
               ...
               CALL PDLAMVE_( 'All', JW+IROFFH, JW+IROFFH, T, 1, 1,
     ...
     $              LIWORK-NSEL, INFQR )
               CALL PDLAMVE_( 'All', JW+IROFFH, JW+IROFFH, WORK(IPT0),
     $              1, 1, DESCTZ0, T, 1, 1, DESCT, WORK(IPW0) )
               CALL PDLAMVE_( 'All', JW+IROFFH, JW+IROFFH, WORK(IPZ0),
     $              1, 1, DESCTZ0, V, 1, 1, DESCV, WORK(IPW0) )
            END IF
         END IF
      ELSE
*
...
*
     ...
     $   INFQR = INFQR - IROFFH
*
*     PDTRORD_ needs a clean margin near the diagonal.
*
      DO 50 J = 1, JW - 3
...
*
*     Clean up the array SELECT for PDTRORD_.
*
      DO 60 J = 1, NSEL
         IWORK( J ) = 0
 60   CONTINUE
...
*
 ...
 110              CONTINUE
                  IWORK( IFST+IROFFH ) = 1
                  CALL PDTRORD_( 'Vectors', IWORK, PAR, JW+IROFFH, T, 1,
...
*                 rely on the output of PDTRORD_.
*
                  IWORK( IFST+IROFFH ) = 0
                  IWORK( LILST ) = 1
                  LILST = LILST + 1
